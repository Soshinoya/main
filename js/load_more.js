...
(function($) {
	jQuery.fn.portfolio_addon = function (addon_options) {
        ...
        $(".gallery__button").on('click', function(){
            ...
            // Creating elements
            while(img_per_load) {
            	$newEls += '<div class="av-added gallery__item col-xs-12 col-sm-6 ' + addon_options.items[alreadyAdded].filter + '" style="background-image: url(' + addon_options.items[alreadyAdded].backgroundImage + ')"><div class="gallery__item__hover"><span class="item__hover__line"></span><span class="item__hover__tag">' + addon_options.items[alreadyAdded].tagName + '</span>' + '<h3 class="item__hover__header"><a target="_blank" href="' + addon_options.items[alreadyAdded].headingHref + '">' + addon_options.items[alreadyAdded].heading + '</a></h3><h4 class="item__hover__header-small">' + addon_options.items[alreadyAdded].headingSmall + '</h4><a href="' + addon_options.items[alreadyAdded].img + '" rel="nofollow" class="item__hover__zoom" title="' + addon_options.items[alreadyAdded].title + '"><img src="' + addon_options.items[alreadyAdded].img + '" alt>+&nbsp;Zoom</a></div></div>\n';
